...
# What mental code did you build:
...
#COMPLETE VARIATION WITHOUT AI

def ogba_advanced(text):
    ...
    Returns:
    - True if palindrome
    - If not, returns the 0-based index (left pointer) where it fails
    

    # Step 1: Normalize text
    text = str(text).lower()  # Convert to lowercase
    allowed_chars = string.ascii_lowercase + string.digits  # Only letters and digits
    cleaned_text = "".join(c for c in text if c in allowed_chars)  # Remove spaces/punctuation
    ...
    while left < right:
        if cleaned_text[left] != cleaned_text[right]:
            return left  # Return position where it fails
        left += 1
        right -= 1
